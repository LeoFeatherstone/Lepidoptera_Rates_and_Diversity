...
papi_hosts_summ1 <- papi_hosts_final %>% 
    group_by(Genus, Species) %>% 
    summarise(Family = first(Family), N_host_sp = length(unique(Host_name))) %>% 
    summarise(Family = first(Family), Hosts_pgen = sum(N_host_sp > 1, na.rm = T) / n(), Hosts_mean = mean(N_host_sp, na.rm = T))

papi_hosts_summ2 <- papi_hosts_final %>% 
    group_by(Genus) %>% 
    summarise(Host_families = length(unique(Host_family)), Host_species = length(unique(Host_name)))

papi_hosts_summ <- papi_hosts_summ1 %>% 
    left_join(papi_hosts_summ2, by = "Genus")
...
papi_hosts_pd <- apply(
    papi.genus.names, c(1, 2),
    function (x) {
        sum(keep.tip(
            angiosperm.tree,
            intersect(
                angiosperm.tree$tip.label,
                gsub(" ", "_", papi_hosts_final %>% filter(Genus == x) %>% pull(Host_name) %>% unique)
            ))$edge.length)
    }
)

papi.host.contrasts <- rbind(
    apply(papi.genus.names, c(1, 2), function (x) papi_hosts_summ_final %>% filter(Genus == x) %>% pull(Hosts_pgen)), 
    apply(papi.genus.names, c(1, 2), function (x) papi_hosts_summ_final %>% filter(Genus == x) %>% pull(Hosts_mean)), 
    apply(papi.genus.names, c(1, 2), function (x) papi_hosts_summ_final %>% filter(Genus == x) %>% pull(Host_families)), 
    apply(papi.genus.names, c(1, 2), function (x) papi_hosts_summ_final %>% filter(Genus == x) %>% pull(Host_species)), 
    papi_hosts_pd
)
...
papi.hosts.out.final.t <- papi.hosts.out.t %>% 
...
papi.analysis.blen <- papi.hosts.out.final.t %>% 
  mutate(diff = log(blen_first) - log(blen_last)) %>% 
  welch.test(diff, exp(Pair_age)) %>% 
  select(-diff) %>% 
...
papi.analysis.dN <- papi.hosts.out.final.t %>% 
  mutate(diff = log(dN_first) - log(dN_last)) %>% 
  welch.test(diff, exp(Pair_age)) %>% 
  select(-diff) %>% mutate_if(is.numeric, log) %>% 
...
papi.analysis.dS <- papi.hosts.out.final.t %>% 
...
papi.analysis.hosts <- papi.hosts.out.final.t %>% 
  mutate_if(is.numeric, log) %>% 
...
## FINAL OUTPUT - LINEAR MODEL RESULTS AND PLOTS

papi.blen.lm <- summary(lm(data=papi.analysis.blen, formula=N_spp_std ~ blen_std-1))$coeff
papi.dN.lm <- summary(lm(data=papi.analysis.dN, formula=N_spp_std ~ dN_std-1))$coeff
papi.dS.lm <- summary(lm(data=papi.analysis.dS, formula=N_spp_std ~ dS_std-1))$coeff

papi.blen.hosts.lm <- summary(lm(data = papi.analysis.blen, formula=Host_families_std ~ blen_std-1))$coeff
...
papi.hosts.pgen.lm <- summary(lm(data=papi.analysis.hosts, formula=N_spp_std ~ Hosts_pgen_std-1))$coeff
papi.hosts.mean.lm <- summary(lm(data=papi.analysis.hosts, formula=N_spp_std ~ Hosts_mean_std-1))$coeff
papi.hosts.pd.lm <- summary(lm(data=papi.analysis.hosts %>% filter(is.finite(Hosts_pd_std)), formula=N_spp_std ~ Hosts_pd_std-1))$coeff

papi.hosts.fam.pl <- ggplot(papi.analysis.hosts) + geom_point(aes(y = N_spp_std, x = Host_families_std)) + ylab("Genus contrast in clade size") +  xlab("Genus contrast in number of host plant families") + expand_limits(x=6) + theme(panel.background = element_blank(), axis.line = element_line(colour = "black", size = 0.5), text = element_text(family="serif")) + geom_abline(linetype = "dashed", size=1, colour="grey80", slope=papi.hosts.fam.lm[1], intercept = 0)
papi.hosts.spp.pl <- ggplot(papi.analysis.hosts) + geom_point(aes(y = N_spp_std, x = Host_species_std)) + ylab("Genus contrast in clade size") +  xlab("Genus contrast in number of host plant species") + expand_limits(x=6) + theme(panel.background = element_blank(), axis.line = element_line(colour = "black", size = 0.5), text = element_text(family="serif")) + geom_abline(linetype = "dashed", size=1, colour="red", slope=papi.hosts.spp.lm[1], intercept = 0)
papi.hosts.pgen.pl <- ggplot(papi.analysis.hosts) + geom_point(aes(y = N_spp_std, x = Hosts_pgen_std)) + ylab("Genus contrast in clade size") +  xlab("Genus contrast in proportion of generalist species") + expand_limits(x=6) + theme(panel.background = element_blank(), axis.line = element_line(colour = "black", size = 0.5), text = element_text(family="serif")) + geom_abline(linetype = "dashed", size=1, colour="grey80", slope=papi.hosts.pgen.lm[1], intercept = 0)
papi.hosts.mean.pl <- ggplot(papi.analysis.hosts) + geom_point(aes(y = N_spp_std, x = Hosts_mean_std)) + ylab("Genus contrast in clade size") +  xlab("Genus contrast in mean host plant species per Lepidopteran species") + expand_limits(x=6) + theme(panel.background = element_blank(), axis.line = element_line(colour = "black", size = 0.5), text = element_text(family="serif")) + geom_abline(linetype = "dashed", size=1, colour="grey80", slope=papi.hosts.mean.lm[1], intercept = 0)
papi.hosts.pd.pl <- ggplot(papi.analysis.hosts %>% filter(is.finite(Hosts_pd_std))) + geom_point(aes(y = N_spp_std, x = Hosts_pd_std)) + ylab("Genus contrast in clade size") +  xlab("Genus contrast in host plant phylogenetic diversity") + expand_limits(x=6) + theme(panel.background = element_blank(), axis.line = element_line(colour = "black", size = 0.5), text = element_text(family="serif")) + geom_abline(linetype = "dashed", size=1, colour="red", slope=papi.hosts.pd.lm[1], intercept = 0)

papi.cordat <- cor(papi.analysis.hosts %>% select_at(vars(contains("std"), -contains("rate"), -contains("Hosts_pd"), -contains("time"))) %>% filter(!is.na(Host_families_std)))
papi.cordat[upper.tri(papi.cordat)] <- NA
ggplot(melt(papi.cordat), aes(x=Var1, y=Var2)) + geom_tile(aes(fill=value, text=value)) + geom_text(aes(Var1, Var2, label = round(value,2)), size = 5) + scale_color_gradient2(na.value="grey80", low="red", mid="white", high="skyblue", midpoint=0, aesthetics="fill", name="Pearson", limit=c(-1,1))  + theme(axis.text.x = element_text(hjust=1, angle=45))
